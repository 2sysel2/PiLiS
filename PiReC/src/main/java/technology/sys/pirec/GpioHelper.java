...
public class GpioHelper {
	...
	public GpioHelper(String configFilePath) {
		gpioController = GpioFactory.getInstance();
		configureHelper(ControllerConfigHelper.getConfig(configFilePath));
	}

	private void configureHelper(ControllerConfig config) {
		createManualOveride(config);
		config.getRelayPinNames().stream().forEach(rpn -> addPin(rpn));

	}

	private void createManualOveride(ControllerConfig config) {
		GpioPinDigitalInput in = gpioController.provisionDigitalInputPin(
				RaspiPin.getPinByName(config.getOverridePinName()),
				PinPullResistance.PULL_DOWN);

		in.addListener((GpioPinListenerDigital) (
				GpioPinDigitalStateChangeEvent event) -> {
			LocalDateTime now = LocalDateTime.now();
			if (event.getState().isHigh() && ChronoUnit.SECONDS
					.between(lastManualButtonPress, now) >= 1) {
	...
	private Map<String, Boolean> getPinStateMap() {
		...
		pinMap.entrySet().stream().forEach((entry) -> {
