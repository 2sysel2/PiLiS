...
public class LightControllerPlugin implements ILightControllerPlugin {
	...
	private static final Logger LOG = LogManager
			.getLogger(LightControllerPlugin.class);

	@Override
	public void turnOn(Light light) {
		GpioRequest request = new GpioRequest(Action.TURN_ON,
				light.getPinName());
		GpioResponse response = sendRequest(request, light.getControllerIp());
		if (response != null) {
			applyStateFromReponse(response, light);
		}
	...
	public void turnOff(Light light) {
		GpioRequest request = new GpioRequest(Action.TURN_OFF,
				light.getPinName());
		GpioResponse response = sendRequest(request, light.getControllerIp());
		if (response != null) {
			applyStateFromReponse(response, light);
	...
	public void toggle(Light light) {
		GpioRequest request = new GpioRequest(Action.TOGGLE,
				light.getPinName());
		GpioResponse response = sendRequest(request, light.getControllerIp());
		if (response != null) {
			applyStateFromReponse(response, light);
	...
	private GpioResponse sendRequest(GpioRequest request, String controllerIp) {
		try (Socket socket = new Socket(controllerIp, 50000)) {
			...
			GpioResponse response = XmlHelper
					.unmarshallResponse(responseString);
			return response;
	...
	private void applyStateFromReponse(GpioResponse response, Light light) {
